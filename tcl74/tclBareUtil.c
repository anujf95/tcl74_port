				 ...
				 * but it will never get any data. */
{
	...
	*errFilePtr = -1;
	...
	if (input != NULL) {
	    ...
	    tmpnam(inName); //XXX: Done will not effect since pipes are not supported. Function will not reach this point
	...
	*outPipePtr = pipeIds[0];
	...
	tmpnam(errName); //XXX: Done will not effect since pipes are not supported. Function will not reach this point
